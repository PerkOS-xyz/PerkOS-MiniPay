...
import { USDT } from "../lib/tokenAddresses";
...
export function ExternalTransfer({ address }: { address: string }) {
  ...
  const balanceDisplay = balance === undefined
    ? "—"
    : Number(formatUnits(balance, USDT.decimals)).toLocaleString(undefined, { maximumFractionDigits: 6 });
  ...
  function message(code: TransferValidationError) {
    switch (code) {
      ...
      case "too-many-decimals": return tr("USDT supports up to 6 decimal places.", "USDT admite máximo 6 decimales.", "USDT aceita no máximo 6 casas decimais.");
      case "insufficient-balance": return tr("You do not have enough USDT.", "No tienes suficiente USDT.", "Você não tem USDT suficiente.");
  ...
  function validate() {
    const result = validateExternalTransfer({ sender, recipient, amount, decimals: USDT.decimals, balance });
    if (result.error) {
  ...
  async function send() {
    ...
    setStage("sending");
    try {
      setHash(await pay(valid.recipient, amount.trim(), USDT));
      setStage("sent");
      setTimeout(() => balanceQuery.refetch(), 5000);
    } catch (cause) {
  ...
  return (
    <section className="flex flex-col gap-5">
      <div>
        <p className="text-xs font-medium uppercase tracking-[0.16em] text-[var(--accent)]">USDT · Celo</p>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs text-[var(--muted)]">{tr("Available in MiniPay", "Disponible en MiniPay", "Disponível no MiniPay")}</p>
            <p className="mt-1 text-xl font-semibold">{balanceDisplay} USDT</p>
          </div>
      ...
      {isMiniPay === false ? (
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <label className="flex flex-col gap-1.5 text-sm">
            <span>{tr("Amount in USDT", "Monto en USDT", "Valor em USDT")}</span>
            <input aria-label={tr("Amount in USDT", "Monto en USDT", "Valor em USDT")} value={amount} onChange={(event) => { setAmount(event.target.value); setError(null); }} inputMode="decimal" placeholder="0.00" className="h-12 rounded-xl border border-white/15 bg-black/25 px-3 text-base" />
          </label>
          <button type="button" onClick={() => { if (validate()) setStage("review"); }} disabled={!ready || balance === undefined} className="h-12 rounded-xl bg-[var(--accent)] px-4 font-medium text-white disabled:opacity-50">
