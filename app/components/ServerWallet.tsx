...
export function ServerWallet({ address }: { address: string }) {
    ...
    ensureServerWallet()
      ...
      .catch((e) => {
        ...
        setMsg(
          e instanceof Error
            ? e.message
            : tr("Couldn't set up your business wallet.", "No se pudo configurar la wallet del equipo.", "Não foi possível configurar a carteira do negócio."),
        );
      });
  }, [locale]);
  ...
  async function deposit(usd: number) {
    ...
    if (!token) {
      setMsg(tr(`You need at least $${usd} in USDT, cUSD or USDC to deposit.`, `Necesitas al menos $${usd} en USDT, cUSD o USDC para depositar.`, `Você precisa de pelo menos $${usd} em USDT, cUSD ou USDC para depositar.`));
      return;
    }
    setBusy(true);
    try {
      setMsg(tr(`Depositing $${usd} ${token.symbol}…`, `Depositando $${usd} ${token.symbol}…`, `Depositando $${usd} ${token.symbol}…`));
      await pay(addr, String(usd), token);
      setMsg(tr("Deposit sent. It will appear here shortly.", "Depósito enviado. Aparecerá aquí en unos momentos.", "Depósito enviado. Ele aparecerá aqui em breve."));
      setTimeout(refetch, 5000);
    } catch (e) {
      setMsg(e instanceof Error ? e.message : tr("Deposit failed. Please try again.", "El depósito falló. Inténtalo nuevamente.", "O depósito falhou. Tente novamente."));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold">{tr("Business wallet", "Wallet del negocio", "Carteira do negócio")}</h1>
        <p className="text-sm text-[var(--muted)]">
          {tr(
            ...
            "Uma carteira separada para futuros pagamentos do negócio na Celo. Os depósitos estão disponíveis; os gastos automáticos não estão habilitados. Sua carteira pessoal permanece privada e sua chave nunca é compartilhada.",
          )}
        </p>
      </div>
      ...
      {state === "error" ? (
        <div className="rounded-2xl border border-red-300/20 bg-red-400/5 p-4">
          <p className="text-sm font-medium text-red-200">
            {tr("We couldn't load the business wallet.", "No pudimos cargar la wallet del negocio.", "Não conseguimos carregar a carteira do negócio.")}
          </p>
        ...
        <>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs text-[var(--muted)]">{tr("Balance on Celo", "Balance en Celo", "Saldo na Celo")}</p>
            ...
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-white/10 bg-black/20 px-3 py-2">
              <span className="min-w-0 flex-1 truncate font-mono text-xs">
                {state === "loading" ? tr("Setting up your wallet…", "Configurando tu wallet…", "Configurando sua carteira…") : short}
              </span>
              <button
                type="button"
                onClick={copy}
            ...
            <p className="mt-1 text-[11px] text-[var(--muted)]">
              {tr("Deposit USDT, cUSD or USDC to this address, or use the buttons below.", "Deposita USDT, cUSD o USDC en esta dirección o usa los botones de abajo.", "Deposite USDT, cUSD ou USDC neste endereço ou use os botões abaixo.")}
            </p>
          </div>

          <div>
            <p className="mb-2 text-sm font-medium">{tr("Deposit", "Depositar", "Depositar")}</p>
            ...
            <p className="mt-2 text-xs text-[var(--muted)]">
              {tr("Sent from your wallet · pay with USDT, cUSD or USDC", "Enviado desde tu wallet · paga con USDT, cUSD o USDC", "Enviado da sua carteira · pague com USDT, cUSD ou USDC")}
            </p>
          </div>
